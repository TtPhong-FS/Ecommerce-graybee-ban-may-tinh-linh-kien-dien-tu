...
export const PcCarousel = () => {
  const pcs = useSelector((state) => state.carousel.pcs)
  ...
  return (
    <div className="box">
      <h2 className="title mb-4">PC bán chạy</h2>
      <Swiper
          ...
          <SwiperSlide key={pc.id}>
            <div className="carousel-container" key={index}>
              <div className="carousel-box">
                <div onClick={() => toDetail({ id: pc.id, name: pc.name })} className="carousel-container-img">
                  <img width="100%" height={180} src={pc.thumbnail} alt={pc.name} />
                </div>
                <div className="carousel-container-info">
                  <div onClick={() => toDetail({ id: pc.id, name: pc.name })} className="cursor-pointer h-[5rem]">
                    <p className="carousel-content-text max-sm:text-[12.8px] line-clamp-3">{pc.name}</p>
                  </div>
                  <div className="h-[1.5rem]">
                    <del className="font-medium text-sm text-gray-500 max-sm:text-[11px]">
                      {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(pc.price)}
                    </del>
                    <span className="font-medium text-lg font-sans ml-1 text-red-500 max-sm:text-[12px]">
                      {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(pc.finalPrice)}
                    </span>
                  </div>
                </div>
                <div className="carousel-container-button sm:mt-4">
                  <IconButton onClick={() => handleAddToFavourites(pc.id)}>
                    {isPresentInFavorites(favourites, pc.id) ? (
                  ...
                  <Button
                    ...
                    onClick={() => handleAddItemToCart(pc.id, 1)}
                    type="primary"
                  />
                </div>
              </div>
